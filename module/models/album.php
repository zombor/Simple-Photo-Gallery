...
class Album_Model extends Auto_Modeler_ORM {
	...
	public function find_photos($page_number)
	{
		...
		return $this->db->from('photos')->limit($per_page, ($page_number*$per_page-$per_page))->orderby(array('photo_order' => 'ASC'))->where(array('album_id' => $this->data['id']))->get()->result(TRUE, 'Photo_Model');
	}
}