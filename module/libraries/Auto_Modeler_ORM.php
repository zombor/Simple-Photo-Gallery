...
class Auto_Modeler_ORM_Core extends Auto_Modeler
{
	...
	public function find_related($key, $where = array(), $orderby = array())
	{
		if (isset($this->aliases[$key]))
			$key = $this->aliases[$key];
		...
		if ($temp->has_attribute(inflector::singular($table_name).'_id')) // Look for a one to many relationship
			return $this->db->from($key)->orderby($orderby)->where($where + array(inflector::singular($table_name).'_id' => $this->data['id']))->get()->result(TRUE, inflector::singular(ucwords($key)).'_Model');
	...
	public function find_parent($key = NULL, $where = array(), $orderby = array())
	{
		if (isset($this->aliases[$key]))
			$key = $this->aliases[$key];
		$table_name = isset($this->aliases[$this->table_name]) ? $this->aliases[$this->table_name] : $this->table_name;
		...
		{
			return $this->db->from(inflector::plural($table_name))->orderby($orderby)->where($where + array('id' => $this->data[$key.'_id']))->get()->result(TRUE, ucwords($table_name).'_Model');
		}
		else
		{
